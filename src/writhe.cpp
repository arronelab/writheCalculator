...
point writhe::dboldLk(std::vector<point>& pointList,std::vector<point>& pointList2,int size,int size2,int i,int j){
  i = i%(size-1);
  j = j%(size2-1);
...
double writhe::muLink(std::vector<point>& pointList,std::vector<point>& pointList2,int size,int size2,int i, int j){
  int iplus = i+1;
  int iminus = i-1;
  if(i==0){
    iminus=size-1;
...
double writhe::DI(std::vector<point>& pointList){
...
double writhe::AbsDI(std::vector<point>& pointList){
  //pointList.push_back(pointList[0]);
  int listSize = pointList.size();
  double sigsum=0.0;
  for(int i=0;i<listSize-1;i++){
...
std::pair<double,double> writhe::sumWr(int& iv,int& jv,std::vector<std::vector<double> >& wijVals){
  double sigsum=0.0;
...
std::pair<std::vector<std::vector<point> >,std::vector<std::vector<point> > >  writhe::getWritheFingerprints(std::vector<point>& pointList){
  int listSize = pointList.size();
...
double writhe::DIClosed(std::vector<point>& pointList){
  pointList.push_back(pointList[0]);
  int listSize = pointList.size();
  double sigsum=0.0;
  for(int i=0;i<listSize-1;i++){
...
double writhe::DIClosedLk(std::vector<point>& pointList,std::vector<point>& pointList2){
  int listSize = pointList.size();
...
double writhe::compare(std::vector<point>& sec1,std::vector<point>& sec2,int& maxSz){
  double sum=0.0;
...
bool writhe::checkOverlap(std::pair<std::pair<int,int>,std::pair<int,int> >& p1,std::pair<std::pair<int,int>,std::pair<int,int> >& p2){
...
bool writhe::checkOverlap(std::pair<int,int>& p1,std::pair<int,int>& p2){
...
std::pair<bool,double>  writhe::checkOverlapLargest(std::pair<int,int>& p1,std::pair<int,int>& p2){
  std::pair<bool,int> out;
...
std::vector<std::pair<std::pair<int,int>,std::pair<int,int> > > writhe::compareFingerPrints(std::vector<std::vector<point> >& fp1In,int& len1In,std::vector<std::vector<point> >& fp2In,int& len2In,double& cutOff){
  int maxZ = 8;
  ...
  if(allPairs.size()>0){
    ...
    /* for(int i= 0;i<nonRepeats.size();i++){
       for(int j= 0;j<nonRepeats[i].size();j++){
	 std::cout<<" nr "<<nonRepeats[i][j].first.first<<" "<<nonRepeats[i][j].first.second<<" "<<nonRepeats[i][j].second.first<<" "<<nonRepeats[i][j].second.second<<"\n";
       }
       }*/
...
std::vector<std::pair<std::pair<int,int>,std::pair<int,int> > > writhe::finalSort(std::vector<std::vector<std::pair<std::pair<int,int>,std::pair<int,int> > > > & prSet){
  std::vector<std::pair<std::pair<int,int>,std::pair<int,int> > >  bestCombo;
  if(prSet.size()>0){
    int noSlices=prSet.size();
    //std::cout<<" how many slices ? "<<noSlices<<"\n";
    std::vector<int> permSizes;
    int permtot=1.0;
    for(int i=0;i<noSlices;i++){
      //std::cout<<"whats the product?"<<prSet[i].size()<<"\n";
...
std::pair<int,std::vector<std::pair<std::pair<int,int>,std::pair<int,int> > > > writhe::findBiggestOverlap(std::vector<std::vector<std::pair<std::pair<int,int>,std::pair<int,int> > > >& separatedCombo,std::vector<int>& perm){
  int outLen=0;
  ...
  for(int i=0;i<perm.size()-1;i++){
    for(int j=i+1;j<perm.size();j++){
      ...
      //std::cout<<separatedCombo[i][perm[i]].second.first<<" "<<separatedCombo[i][perm[i]].second.second<<" "<<separatedCombo[j][perm[j]].second.first<<" "<<separatedCombo[j][perm[j]].second.second<<"\n";
      if(chk1.first==true || chk2.first==true){
	std::pair<int,int> pr(i,j);
	//std::cout<<"classed as forbidden "<<i<<" "<<j<<" "<<pr.first<<" "<<pr.second<<"\n";
...
std::vector< std::vector<int> > writhe::getAllSubsets(std::vector<int>& set)
{
